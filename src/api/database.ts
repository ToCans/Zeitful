...
export async function addTask(task: WorkTask): Promise<DatabaseActionResponse> {
	try {
		...
		// Check if topic already exists
		const existing = db.exec(
			'SELECT * FROM WorkTask WHERE task_name = "' + task.task_name.replace(/"/g, '""') + '"'
		);

		if (existing.length > 0) {
			return {
				status: 'Failure',
				message: `Take "${task.task_name}" already exists — skipping insert.`,
			};
		}

		db.run(
			`INSERT INTO WorkTask (id, topic_id, task_name, status)
			 VALUES (?, ?, ?, ?)`,
			[
				task.id,
...
export async function addWorkEntry(workEntry: WorkEntry): Promise<DatabaseActionResponse> {
	try {
        ...
        (id, topic, task_name, duration, completion_time) 
        VALUES (?, ?, ?, ?, ?)`,
			[
				workEntry.id,
				workEntry.topic,
		...
		return {
			status: 'Success',
			message: `Task "${workEntry.task_name}" inserted into database.`,
		};
	} catch (e) {
		return {
			status: 'Failure',
			message: `Task "${workEntry.task_name}" wasn't inserted into database. ${e}`,
		};
	}
}
...
export async function getTasks(): Promise<WorkTask[]> {
	const db = await getDatabase();
	const res = db.exec(`
    SELECT t.id, t.topic_id, t.task_name, t.status
    FROM WorkTask t
  `);

	if (res.length === 0) return [];

	return res[0].values.map(([id, topic_id, task_name, status]) => ({
		id: id as string,
