...
const TaskFilterButton = ({ isActive, value, setWorkTaskStatus }: FilterButtonProps) => {
    ...
    return (
        <button className={`h-6 rounded-lg border-2 ${isActive ? 'opacity-100' : 'opacity-50 hover:opacity-100'} ${settings.darkMode ? 'border-zinc-700 text-zinc-100' : 'border-zinc-200 text-black'} px-1`} onClick={() => (setWorkTaskStatus(value))}>
